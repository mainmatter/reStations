...
/*
TODO apparently, this doesn't map well to reality. For instance,
the station named "konstanz" has distribusion_id "DEKONKBA", but its value
for distribusion_is_enabled is "f".
*/
pub enum Id {
    Enabled(String),
    #[default]
...
impl rusqlite::ToSql for Id {
    fn to_sql(&self) -> rusqlite::Result<rusqlite::types::ToSqlOutput<'_>> {
        match self {
            Id::Enabled(id) => id.to_sql(),
            Id::Disabled => Ok(rusqlite::types::ToSqlOutput::from(rusqlite::types::Null)),
        }
    }
}
...
impl rusqlite::types::FromSql for Id {
    fn column_result(value: rusqlite::types::ValueRef<'_>) -> rusqlite::types::FromSqlResult<Self> {
        let s = value.as_str_or_null()?;
        Ok(s.clone()
            .map(ToOwned::to_owned)
            .map(Id::Enabled)
            .unwrap_or(Id::Disabled))
    }
}

#[derive(thiserror::Error, Debug)]
enum IdProxyError {
    #[error("The ID was set, but it was marked as disabled.")]
    IdSetButDisabled,
    #[error("Marked as enabled, but the ID was empty")]
    EnabledButIdEmpty,
}
...
impl TryFrom<IdProxy> for Id {
    type Error = IdProxyError;

    fn try_from(IdProxy { id, is_enabled }: IdProxy) -> Result<Self, Self::Error> {
        match (is_enabled, id.is_empty()) {
            (true, false) => Ok(Id::Enabled(id)),
            (false, true) => Ok(Id::Disabled),
            (true, true) => Err(IdProxyError::EnabledButIdEmpty),
            (false, false) => Err(IdProxyError::IdSetButDisabled),
        }
    }
}
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(field) = map.next_key::<&'de str>()? {
                    match dbg!(field) {
                        ...
                        "distribusion_id" => distribusion.id(dbg!(map.next_value()?)).yeet(),
                        "distribusion_is_enabled" => {
                            distribusion.parse_is_enabled(dbg!(map.next_value()?))?
                        }
...
into_deser_error_impl!(IdProxyBuilderError);
